...
func NewConsumerGroup(brokers []string, groupID string, log *logger.StandardLogger) *consumerGroup {
...
func (c *consumerGroup) ConsumeTopic(groupTopics []string, poolSize int, worker Worker) {
	...
	defer func() {
		if err := r.Close(); err != nil {
			c.log.WithKeyword(ctx, fmt.Sprintf("consumerGroup.r.Close: %v", err)).Warn()
		}
	}()
	c.log.WithKeyword(
		ctx,
